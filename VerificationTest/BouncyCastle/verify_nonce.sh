...
# Usage: ./verify_nonce.sh              # Run basic test (no nonce)
#        ./verify_nonce.sh <nonce_hex>  # Verify hardware nonce
...
# Case 1: No arguments - run basic test
if [ $# -eq 0 ]; then
    echo "Running basic SHA3 test (no nonce)..."
    echo ""
    java -Djava.library.path=. -cp "$BCPROV_JAR:." Java_HW_test
    exit 0
fi

# Case 2: Nonce provided - verify hardware result
NONCE_HEX="$1"
...
java -Djava.library.path=. -cp "$BCPROV_JAR:." Java_HW_test "$NONCE_HEX"
